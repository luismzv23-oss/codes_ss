    ...
    <style>
        ...
        * {
        ...
        html.light-theme .banner p {
            color: #334155;
            /* Texto con mayor contraste y legibilidad */
            text-shadow: none;
        }
    ...
    <header class="topbar">
        ...
        <div class="user-nav">
            ...
            <?php if (session()->get('isLoggedIn')): ?>
                ...
                <a href="/sportsbook/profile" class="user-badge"
                    style="text-decoration:none;cursor:pointer;transition:all 0.2s;"
    ...
    <main class="layout-grid">
        ...
        <section class="center-col">
            <div class="banner">
                ...
                <p>Prepartido, calendario en vivo para seguimiento, tickets auditados y controles de juego responsable.
                </p>
            </div>
            ...
            <form class="sports-toolbar" method="get" action="/">
                ...
                <label class="search-box">
                    <i data-lucide="search"
                        style="width:16px;height:16px;color:var(--text-muted);margin-left:0.25rem;"></i>
                    <input name="q" value="<?= esc($search ?? '') ?>"
                        placeholder="Buscar equipo, liga, país, deporte o competición...">
                    <button type="submit">Buscar</button>
                </label>
                <?php if (!empty($search)): ?>
                    <a class="tab-link" href="<?= sportsbookUrl(['q' => null]) ?>"
            ...
            <nav class="tabs" aria-label="Filtros de eventos">
                <?php foreach ($tabItems as $tabKey => $tabLabel): ?>
                    <a class="tab-link <?= $activeTab === $tabKey ? 'active' : '' ?>"
                        href="<?= sportsbookUrl(['tab' => $tabKey]) ?>"><?= esc($tabLabel) ?></a>
                <?php endforeach; ?>
            </nav>

            <?php if (!empty($champion)): ?>
                <div
                    style="position:relative; overflow:hidden; margin-bottom:1.25rem; border:1px solid rgba(251,191,36,0.42); border-radius:0.75rem; padding:1.25rem 1.5rem; background:linear-gradient(135deg, rgba(251,191,36,0.20), rgba(30,41,59,0.96) 58%, rgba(16,185,129,0.15));">
                    <div
                        style="position:absolute; right:1.25rem; top:0.35rem; font-size:5rem; line-height:1; color:rgba(251,191,36,0.22);">
                        🏆</div>
                    <div
                        style="font-size:0.78rem; text-transform:uppercase; letter-spacing:0.12em; color:#fbbf24; font-weight:900;">
                        Felicitaciones Campeón</div>
                    <div
                        style="display:flex; align-items:center; gap:0.7rem; margin-top:0.45rem; font-family:Outfit, sans-serif; font-size:1.65rem; font-weight:900;">
                        <?= flagMarkup($champion['flag'] ?? null) ?>
                        <span><?= esc($champion['team']) ?></span>
                    </div>
                    <div style="margin-top:0.25rem; color:var(--text-muted); font-size:0.92rem;">Campeón de la Copa Mundial
                        de Fútbol 2026</div>
                </div>
            <?php endif; ?>
            ...
            <?php foreach ($events as $event): ?>
                ...
                <div class="event-card" data-event-id="<?= (int) $event['id'] ?>">
                    <button class="favorite-btn" type="button" onclick="toggleFavorite(<?= (int) $event['id'] ?>, this)"
            ...
            <section
                style="margin-top:1.5rem;border:1px solid var(--border);border-radius:0.8rem;background:var(--bg-panel);padding:1rem;display:grid;gap:0.85rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap;">
                    <div>
                        <div
                            ...
                            Confianza y juego responsable</div>
                        <div style="color:var(--text-muted);font-size:0.86rem;margin-top:0.25rem;">Plataforma 18+, KYC
                            para retiros, limites de riesgo y reglas publicas de liquidacion.</div>
                    </div>
                ...
                <div
                    style="color:var(--text-muted);font-size:0.76rem;line-height:1.45;border-top:1px solid var(--border);padding-top:0.75rem;">
                    Operacion sujeta a jurisdiccion habilitada, geolocalizacion y validaciones regulatorias antes de
                    produccion.
                </div>
            </section>
        </section>
        ...
        <aside class="right-col" :class="isRightColOpen ? 'open' : ''">
            ...
            <div class="bet-slip-body">
                ...
                <template x-for="sel in selections" :key="sel.id">
                    <div class="selection-item" x-transition:enter="fade-slide-enter"
